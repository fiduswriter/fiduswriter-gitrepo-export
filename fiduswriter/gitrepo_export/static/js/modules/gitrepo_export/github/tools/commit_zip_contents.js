...
export function commitZipContents(
    ...
    const textCommitFunctions = outputList.map(file => {
        ...
        return () => commitFile(repo, blob, filename, subDir, repoDirCache)
    })
    const commitBinaries = binaryFiles.map(file =>
        get(file.url)
            ...
            .then(blob => {
                const filepathParts = file.filename.split("/")
                const filename = filepathParts.pop()
                const subDir = `${parentDir}${filepathParts.join(
                    "/"
                )}/`.replace(/\/\//, "/")
                return () =>
                    commitFile(repo, blob, filename, subDir, repoDirCache)
            })
    )
                        ...
                        return Promise.all(
                            ...
                            blobs.map((blob, index) => {
                                ...
                                const dir = zipFile.directory
                                    ? [zipFile.directory].concat(filepathParts)
                                    : filepathParts
                                const subDir = `${parentDir}${dir.join(
                                    "/"
                                )}/`.replace(/\/\//, "/")
                                return () =>
                                    commitFile(
    ...
    return Promise.resolve(commitZips)
        .then(zips =>
            Promise.all(commitBinaries).then(binaryCommitFunctions =>
                Promise.all(zips).then(zipCommitFunctions =>
                    textCommitFunctions
                        .concat(binaryCommitFunctions)
                        .concat(zipCommitFunctions)
                        .flat()
                )
            )
        )
        .then(commitFunctions => promiseChain(commitFunctions))
}
